...
const KB: { match: RegExp; reply: string }[] = [
  ...
  { match: /human|support|agent|person/i, reply: "Tap the 'Live support' button at the top — drop your email and a NyayaOS specialist will call you back." },
];

function reply(text: string) {
  for (const k of KB) if (k.match.test(text)) return k.reply;
  return "Got it. A NyayaOS strategist will follow up — meanwhile try a suggested prompt or request live support.";
}
...
export function Chatbot() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>(() => {
    try {
      const saved = localStorage.getItem("nyayaos:chat");
      if (saved) return JSON.parse(saved);
    } catch {
      // ignore
    }
    return [{ role: "bot", text: "Hi, I'm Nyx — the NyayaOS assistant. Ask anything about our services or tap Live support to talk to a human." }];
  });
  const endRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    localStorage.setItem("nyayaos:chat", JSON.stringify(msgs));
  ...
  const submitHandoff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsgs((m) => [
        ...
        {
          role: "system",
          text: `Call request received. We've emailed next steps to ${handoffEmail}. A NyayaOS specialist will call shortly.`,
        },
      ]);
      setTimeout(() => {
